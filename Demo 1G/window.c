...
static GLuint _sphere = 0, _quad = 0, _cube = 0, _torus = 0, _tex = 0, _tId = 0;
...
static void init(void) {
  ...
  initAudio("Darktek.mp3");
}

/*CHARGE UNE TEXTURE A PARTIR D'UN FICHIER IMAGE*/
/*CF SAMPLE DEMOMAKER*/
...
void demoEponge(int temps){
  ...
  if (sommeAigue >= 4000 && moveAigue <= 3)
    moveAigue*=1.02; 
  else
    moveAigue/=1.02;
...
void demoCredit(int temps){
  ...
  if ((temps >= 171019)){
    glUniform1i(glGetUniformLocation(_pId, "temps"), temps-171019);
  }else{
    glUniform1i(glGetUniformLocation(_pId, "temps"), 0);
  }
  ...
  free(f);
  glUniform1i(glGetUniformLocation(_pId, "mobiles"), 1);
...
static void draw(void) {
  ...
  /*BOOLEEN POUR SAVOIR SIN NOUS AVONS DEJA CHANGÉ DE SHADER*/
  static int changeShader = 0;
  ...
  /*UNE FOIS ARRIVÉ A LA FIN DE LA DEMO, ON QUITTE*/
  if(temps >= 224000)
    exit(0);
}
